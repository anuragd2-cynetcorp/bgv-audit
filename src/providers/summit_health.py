...
class SummitHealthProvider(BaseProvider):
    ...
    def __init__(self):
        """Initialize the Summit Health provider."""
    ...
    def extract(self, pdf_path: str) -> ExtractedInvoice:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if not line_items:
                should_try_ocr = True
                logger.info("No line items found with text extraction. Attempting OCR fallback for Summit Health invoice.")
            elif grand_total > 0.0 and abs(grand_total - items_sum) > 0.01:
                should_try_ocr = True
                logger.info(f"Text extraction found {len(line_items)} items with sum ${items_sum:.2f}, but grand total is ${grand_total:.2f}. Attempting OCR fallback for Summit Health invoice.")
    ...
    def _parse_text_lines(self, lines: List[str]) -> List[ExtractedLineItem]:
        """
        Parse text lines into line items using Summit Health-specific logic.
