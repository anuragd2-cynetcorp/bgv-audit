...
class UNAHealthProvider(BaseProvider):
    ...
    def extract(self, pdf_path: str) -> ExtractedInvoice:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if not line_items:
                logger.info("No line items found with text extraction. Attempting OCR fallback for UNA Health invoice.")
                try:
    ...
    def _parse_text_lines(self, lines: List[str]) -> List[ExtractedLineItem]:
        """
        Parse text lines into line items using UNA Health-specific logic.
