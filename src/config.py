import os

class Config:
    # Security
    SECRET_KEY = os.environ.get('SECRET_KEY', 'dev-secret-key')
    
    # Google OAuth
    GOOGLE_CLIENT_ID = os.environ.get('GOOGLE_CLIENT_ID')
    GOOGLE_CLIENT_SECRET = os.environ.get('GOOGLE_CLIENT_SECRET')
    
    # Allow HTTP for OAuth locally (Remove this in production if using HTTPS)
    OAUTHLIB_INSECURE_TRANSPORT = os.environ.get('OAUTHLIB_INSECURE_TRANSPORT', '0')
    DB_ROOT_PATH = "workspaces/bgv-audit"
    
    # File upload configuration
    # Set max content length to 50MB (for large PDF files)
    MAX_CONTENT_LENGTH = 50 * 1024 * 1024  # 50MB in bytes
    
    # Google Cloud Document AI configuration
    # Project ID - auto-detect from GOOGLE_APPLICATION_CREDENTIALS if not set
    DOCUMENT_AI_PROJECT_ID = os.environ.get('DOCUMENT_AI_PROJECT_ID')
    
    # Location for Document AI processor (default: 'us')
    # Options: 'us', 'eu', 'asia', etc.
    DOCUMENT_AI_LOCATION = os.environ.get('DOCUMENT_AI_LOCATION', 'us')
    
    # Processor ID for Document AI OCR processor (optional)
    # If not set, will use the default OCR processor for the project
    DOCUMENT_AI_PROCESSOR_ID = os.environ.get('DOCUMENT_AI_PROCESSOR_ID')

